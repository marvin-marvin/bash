...
export PATH=/usr/local/opt/coreutils/libexec/gnubin:/bin:/usr/local/bin:/usr/sbin:/usr/bin/X11:/usr/bin:/usr/kerberos/bin:/usr/kerberos/sbin:/usr/kerberos/bin:/sbin:/go/bin:/usr/local/go/bin:$GOPATH/bin:.
...
if [[ "$HOSTNAME" == "VPS-Dev-Go" ]] ; then
  PS1="\n$COLOR_GREY[$COLOR_USER${USER}$COLOR_GREY|$COLOR_WS${HOSTNAME}$COLOR_GREY] $COLOR_BLUE\w$COLOR_DEFAULT \[\033[1;32m\]>$COLOR_DEFAULT "
  #PS1="\n$COLOR_GREY[$COLOR_USER${USER}$COLOR_GREY|$COLOR_WS${HOSTNAME}$COLOR_GREY] $COLOR_BLUE\w$COLOR_DEFAULT $(kube_ps1) \[\033[1;32m\]>$COLOR_DEFAULT "
elif [ $? == 0 ]; then
