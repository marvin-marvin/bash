...
_deploy2 () { rsync -avxL --delete -e "ssh -o LogLevel=ERROR -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i ~/bash-keys/acc_rsa -p "$1"" ~/.bash* "$2"@"$3":/tmp >> /tmp/bash_deploy.log; }
...
if [[ "$HOSTNAME" == "vps-ger-fra-1" ]] ; then
    _sshp;
    if ps ax | grep -v grep | grep -v $0 | grep ssh-agent > /dev/null
        then
            eval `ssh-agent`;
